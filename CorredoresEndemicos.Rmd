...
## Evento Enfermedad Tipo Influenza (ETI)
...
### Corredor endémico ETI {.panel-name}
```{r corredor_eti_graf}
graf_eti <- ggplot(corredor_eti,aes(x = semanas_epidemiologicas#, fill=key
)) +
...
### Casos ETI por año {.panel-name}

```{r graf_eti_dengue}

graf_casos_eti <- casos_eti %>%
  filter(anio < 2023) %>%
...
plotly::ggplotly(graf_casos_eti)
